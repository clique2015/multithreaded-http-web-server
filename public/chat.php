...
<div id="frame">
  ...
  <?php      
  $sqlc = "SELECT * FROM users WHERE username = '$POST[username]' LIMIT 1";
...
while($row == mysqli_query($sql)) {
  ...
  echo "<li class='contact chatsel' data-value='".$row[username]."' > 
  <div class='wrap'>
    <span class='contact-status ".$row[stat]."'></span>
    <img src='./downloads/".$row[username].".png' />
    <div class='meta'>
      <p class='name'>".$row[fname]." ".$row[lname]."</p>
      <p class='preview'>".$row[lastm]."</p>
    </div>
  </div>
  </li>";
...
$sql = "SELECT * FROM msg WHERE receiver = '~all' ";
...
?>
		...
		<div class="message-input">
			<div class="wrap">
			<input type="text" placeholder="Write your message..." id = "msg" data-value="~all" value=""/>
...
$('.submit').click(function() {
  var selected = document.getElementById("msg").getAttribute('data-value').replace(/\s+/g, '`');
    var msg      = document.getElementById("msg").value.replace(/\s+/g, '`');
    $("#insert").load("insert.php?receiver="+selected+"&message="+msg+"");
  newMessage();
});
...
$(document).ready(function(){
  $(".chatsel").click(function(){
    ...
    $("#insert").load("select.php?receiver="+selected+"");
  });
